              ...
              {colItems.map((item) => (
                <div key={item.id} draggable onDragStart={(e) => handleDragStart(e, item.id)} onClick={() => setSelected(item)}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-primary/10 to-purple-500/10"><Package className="h-4 w-4 text-primary/50" /></div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setSelected(null)}>
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-card p-6 shadow-2xl" onClick={(e) => e.stopPropagation()} data-testid="kanban-card-detail">
            <div className="flex items-center justify-between"><h2 className="font-heading text-xl font-bold">{selected.orderNumber}</h2><button onClick={() => setSelected(null)} className="rounded-lg p-2 hover:bg-muted"><X className="h-5 w-5" /></button></div>
            <div className="mt-4 space-y-4">
              <div className="flex items-center gap-2 text-sm"><User className="h-4 w-4 text-muted-foreground" /><span className="font-medium">{selected.customerName}</span>{selected.customerPhone && <span className="text-muted-foreground">{selected.customerPhone}</span>}</div>
              <div className="rounded-lg border p-4"><p className="text-xs font-semibold uppercase text-muted-foreground">Item</p><p className="mt-1 font-medium">{selected.productNameSnapshot}</p><p className="text-sm text-muted-foreground">Qty: {selected.qty}</p>{selected.customizationSnapshot && <p className="mt-1 text-xs text-primary">{(() => { try { return JSON.stringify(JSON.parse(selected.customizationSnapshot)) } catch { return selected.customizationSnapshot } })()}</p>}</div>
              <div className="flex justify-between text-sm"><span>Pedido total: <strong>R$ {selected.total.toFixed(2)}</strong></span><span>Prazo est: ~{selected.estimatedHours}h</span></div>
            </div>
          </div>
        </div>
      )}
